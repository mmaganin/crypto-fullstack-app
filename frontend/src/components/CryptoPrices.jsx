...
function createData(name, symbol, price, percent_change_7d, percent_change_30d, market_cap, circulating_supply, id) {
...
const CryptoPrices = () => {
  ...
  const rows = data === null ? null :
    Array.from(data.map((dataIdx) =>
      createData(dataIdx.name, dataIdx.symbol, dataIdx.price, dataIdx.percent_change_7d, dataIdx.percent_change_30d, dataIdx.market_cap, dataIdx.circulating_supply, parseInt(dataIdx.cmc_rank))))
